...
// already.
function stringify(text3) {
    return text3.replace(/([\s\S])[\s\S][\s\S]/g, "$1");
}

// Replace the special unicode symbols with harmless ones. The
// replacement is \ue9c4. This is an arbitrary choice, but nobody
// seems to use it.
function kill_evil_symbols(string) {
    return string.replace(/[\u0950\u232b\u2300\u2326\u06dd]/g, '\ue9c4');
}

// This is really ugly because JS regexps don't have a DOTALL option,
// so I had to replace dots with [\s\S].
var scour_re = /[\s\S]{5}(?:(?:\u2326[\s\S]{4})+\u232b[\s\S]{4})*\u232b[\s\S]{4}(?:[\u232b\u2326][\s\S]{4})*|[\s\S]0[\s\S]0[\s\S](?:[\u232b\u2300\u2326][\s\S]{4})*|([\s\S])[\s\S][\s\S]([\s\S][\s\S])(?:[\u232b\u2326][\s\S]{4})*/g;

// Convert a weave5c to a text3. For speed, this function makes some
// assumptions about the input. For example, all \u2300 atoms are assumed
// to be located after the final \u06dd, which speeds up the regexp and
// does not affect correctness of the rest of the algorithms. For a
// full list, see section 4.6 in the paper.
function scour(weave5c) {
    return weave5c.replace(scour_re, "$1$2");
}
...
// Create a patch5c for the insertion of a string after causing_id,
// with the first id head_id.
function make_insert_patch5c(string, causing_id, head_id) {
    var yarn = head_id[0], yarn_offset = head_id[1];
    var chunks = [];
    
    for (var i = 0; i < string.length; i++) {
        var this_id = yarn + yarn_offset;
        chunks.push(string[i] + causing_id + this_id);
        yarn_offset = unicode_incr(yarn_offset);
        causing_id = this_id;
    }
    return chunks.join('');     // patch5c
}

// Create a patch5c for the deletion of a span in a given text3. The
// starting position (in atoms, not characters) and the length of the
// span (in atoms) define the part that will be deleted. The first
// atom of the span will have the id head_id.
function make_delete_patch5c(text3, start_position, length, head_id) {
    var yarn = head_id[0], yarn_offset = head_id[1];
    start_position *= 3;
    var chunks = [];
    
    for (var i = start_position; i < start_position + length*3; i += 3) {
        var this_id = yarn + yarn_offset;
        chunks.push('\u232b' + text3.substring(i+1, i+3) + yarn + yarn_offset);
        yarn_offset = unicode_incr(yarn_offset);
    }
    return chunks.join('');     // patch5c
}

// Create a save-edits patch5c which will indicate causal awareness of
// one or more atom_ids.
function make_save_edits_patch5c(atom_ids, head_id) {
    var yarn = head_id[0], yarn_offset = head_id[1];
    var chunks = [];
    for (var i = 0; i < atom_ids.length; i++) {
        chunks.push('\u2300' + atom_ids[i] + yarn + yarn_offset);
        yarn_offset = unicode_incr(yarn_offset);
    }
    return chunks.join('');     // patch5c
}
...
// Return the id of the last atom in a given yarn, or null if no such
// atom is found. Like last_atom_id5c, but for text3. Does not include
// any of the atoms which are removed from the weave5c through
// scouring.
function last_atom_id3(text3, yarn) {
    var last_atom = null;
    for (var i = 0; i < text3.length; i += 3) {
	var atom_id = text3.substring(i+1, i+3);
	if (atom_id[0] === yarn)
	    if (last_atom === null || atom_id[1] > last_atom[1])
		last_atom = atom_id;
    }
...
// Apply a patch5c to a weave5c, and return the modified weave5c. This
// is fully general, working with insertion and deletion and change
// saving. FIXME: does not support undo. WARNING: if you use this
// function, you should probably be using the apply functions for each
// patch type instead, and represent composite patches as lists of
// insert, delete, and save-edits patches. This lets you pass weft2 to
// apply_insert_patch, which can speed it up quite a lot.
function apply_patch5c(weave5c, patch5c) {
    // What we need to do is separate the patch into chunks. Each
    // chunk corresponds to an insertion span, a deletion span, or a
    ...
    var INSERT=1, DELETE=2, SAVE=3 // Span types

    function atom_type(i) {
	if (!patch5c[i].match(/[\u0950\u232b\u2300\u2326\u06dd]/)) return INSERT;
	else if (patch5c[i] == '\u232b') return DELETE;
	else if (patch5c[i] == '\u2300') return SAVE;
	else throw "BadPatchError";
    }

    
    var current_span_type = null; // options: INSERT, DELETE, SAVE
    var current_span_start = 0;
    var current_span_end = 0;
    var spans = [];

    for (var i = 0; i < patch5c.length; i += 5) {
	var atype = atom_type(i);
	if (atype === current_span_type) { // Continue a span
	    if (atype === INSERT && patch5c.substring(i+1, i+3) === patch5c.substring(i-2, i))
		current_span_end = i + 5;
	    else if (atype === DELETE || atype === SAVE)
		current_span_end = i + 5
	} else {		// This span is over
	    // Save the old span
	    if (current_span_start !== current_span_end)
                spans.push([patch5c.substring(current_span_start, current_span_end), current_span_type]);
	...
	var span = spans[j][0], span_type = spans[j][1];
	if (span_type === INSERT)
	...
	else if (span_type === DELETE)
	    weave5c = apply_delete_patch(weave5c, span);
	else if (span_type === SAVE)
